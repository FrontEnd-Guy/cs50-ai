...
class MinesweeperAI():
    ...
    def add_knowledge(self, cell, count):
        ...
        for i in range(max(0, cell[0] - 1), min(self.height, cell[0] + 2)):
            for j in range(max(0, cell[1] - 1), min(self.width, cell[1] + 2)):
                if (i, j) != cell and (i, j) not in self.safes and (i, j) not in self.mines:
                    cells_to_inspect.add((i, j))
                elif (i, j) in self.mines:
                    count_mines += 1

        new_sentence = Sentence(cells_to_inspect, count - count_mines)
        ...
        new_inferences = []
        for sentence in self.knowledge:
            if sentence != new_sentence and new_sentence.cells.issubset(sentence.cells):
        ...
        for inferred_sentence in new_inferences:
            if inferred_sentence not in self.knowledge:
                self.knowledge.append(inferred_sentence)
                known_mines = inferred_sentence.known_mines().copy()
                for mine in known_mines:
                    self.mark_mine(mine)
                known_safes = inferred_sentence.known_safes().copy()
                for safe in known_safes:
                    self.mark_safe(safe)
